...
grep -Fq "Native playback bridge using seeded fallback for offline row control action without readable row track" <<< "$STRINGS" || fail "missing row-control fallback seed marker"
grep -Fq "Native playback bridge using seeded fallback for offline row UIApplication action without readable row track" <<< "$STRINGS" || fail "missing row-UIApplication fallback seed marker"
