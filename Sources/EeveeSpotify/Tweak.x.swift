...
struct V91StudifyDownloadSignalGroup: HookGroup { }
struct V91StudifyDownloadSignalLegacyGroup: HookGroup { }
struct V91StudifyDownloadButtonFallbackGroup: HookGroup { }
struct LatestPremiumPatchingGroup: HookGroup { }

func activateV91StudifyDownloadSignalGroup() {
    guard EeveeSpotify.hookTarget == .v91 else { return }

    let targetName = "Offline_ContentOffliningUIImpl.ContentOffliningUIHelperImplementation"
    ...
    if targetClass.instancesRespond(to: legacySelector) {
        V91StudifyDownloadSignalLegacyGroup().activate()
        writeDebugLog("[STUDIFY] Activated 9.1.x legacy playlist download signal hook")
        didActivate = true
    }

    if !didActivate {
        writeDebugLog("[STUDIFY] Offline helper exists, but no known download selector is present")
    }
...
func activatePremiumPatchingGroup() {
    ...
    if EeveeSpotify.hookTarget == .lastAvailableiOS14 {
        ...
        activateV91StudifyDownloadSignalGroup()
